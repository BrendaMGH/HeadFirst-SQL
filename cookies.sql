...
SET IDENTITY_INSERT dbo.cookie_sales ON; 
...
/*SELECT first_name,COUNT(DISTINCT sale_date) FROM cookie_sales
WHERE sales > 0
GROUP BY first_name
ORDER BY COUNT(DISTINCT sale_date) DESC;*/

SELECT TOP 2 first_name, SUM(sales), COUNT(DISTINCT sale_date) FROM cookie_sales
WHERE sales > 0
GROUP BY first_name
ORDER BY SUM(sales) DESC